...
group by month, sum number of delayed arrivals for each station and number of trips to get a percentage of delayed arrivals each month, create racing bar chart that goes up and down for each station. 
```{r, eval=FALSE}
...
mallorcabike <- ggmap(mallorca_map)+
  geom_path(data = mallorca_bike_day7, 
            ...
            size = 1) + 
  #geom_image(aes(bike_image_link))+
...
animate(mallorcabike, duration = 15, end_pause = 5)
...
cases_overtime <- friday_per_10000 %>% 
  ...
  labs(title = "COVID-19 cases per 10,000 people in the US", subtitle = "Date: {frame_along}", fill = "", caption = "Plot created by Erin Franke, data from US Census (2018) and New York Times") +
  theme_map() +
  theme(legend.background = element_blank())+
  transition_reveal(date)
